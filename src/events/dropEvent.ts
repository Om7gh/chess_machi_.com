    ...
    return resetEnPassantPieces
        .map((p) => {
            if (p.x === activePieceCoords.x && p.y === activePieceCoords.y) {
                if (Math.abs(activePieceCoords.x - newX) === 2 && p.type === 'PAWN') {
                    return {
                        ...p,
                        x: newX,
                        y: newY,
...
export {getBoardCoordinates, getCurrentPiece, handleValidMove,updatePiecesForMove, validateTurn}