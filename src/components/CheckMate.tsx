...
export default function CheckMate({winner} : {winner: "WHITE" | "BLACK" | null}) {
  ...
  return (
      ...
      <p className="text-3xl font-bold text-shadow-current">Winner is: <span className={winner === "BLACK" ? "text-slate-950" : "text-slate-100"}>{winner}</span></p>
        </div>
        </div>
  )
}
