...
export const getPawnPossibleMoves = (
  ...
  const startRow = pawn.team === 'ME' ? 1 : 6;
  ...
  if (isCellAccessible(forwardOne.x, forwardOne.y, board)) {
    ...
    if (pawn.x === startRow) {
      ...
      if (
        isCellAccessible(intermediateCell.x, intermediateCell.y, board) &&
  ...
  if (
    isCellOccupiedByOpponent(leftDiagonal.x, leftDiagonal.y, board, pawn.team)
  ) {
    pawnPossibleMoves.push(leftDiagonal);
  }

  if (
    isCellOccupiedByOpponent(rightDiagonal.x, rightDiagonal.y, board, pawn.team)
  ) {
